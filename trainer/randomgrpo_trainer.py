...
class RandomGRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        self.num_generations_reward = args.num_generations_reward
        ...
        if self.use_vllm:
            ...
            self.generation_config = GenerationConfig(
                ...
                do_sample=True,
                temperature=args.temperature,
                num_return_sequences=self.num_generations_reward,
    ...
    def _prepare_inputs(self, inputs: dict[str, Union[torch.Tensor, Any]]) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        if self.args.use_vllm:
            ...
            if self.accelerator.is_main_process:
                ...
                completion_ids = [None] * len(all_prompts_text) * self.num_generations_reward

            # Broadcast the completions from the main process to all processes, ensuring each process receives its
            # corresponding slice.
            completion_ids = broadcast_object_list(completion_ids, from_process=0)
            process_slice = slice(
                self.accelerator.process_index * len(prompts) * self.num_generations_reward,
                (self.accelerator.process_index + 1) * len(prompts) * self.num_generations_reward,
            )
            ...
            prompt_ids = torch.repeat_interleave(prompt_ids, self.num_generations_reward, dim=0)
            prompt_mask = torch.repeat_interleave(prompt_mask, self.num_generations_reward, dim=0)
            ...
            prompt_mask = prompt_mask.repeat_interleave(self.num_generations_reward, dim=0)
        ...
        # Compute the rewards
        prompts = [prompt for prompt in prompts for _ in range(self.num_generations_reward)]  # repeat prompts
            ...
            if isinstance(reward_func, nn.Module):  # Module instead of PretrainedModel for compat with compiled models
                ...
                for key in reward_kwargs:
                    for example in inputs:
                        ...
                        reward_kwargs[key].extend([example[key]] * self.num_generations_reward)
                output_reward_func = reward_func(prompts=prompts, completions=completions, completion_mask=completion_mask, 
                                                 max_completion_length=self.max_completion_length, **reward_kwargs)
        ...
        mean_grouped_rewards = rewards.view(-1, self.num_generations_reward).mean(dim=1)
        std_grouped_rewards = rewards.view(-1, self.num_generations_reward).std(dim=1)
        ...
        mean_grouped_rewards = rewards.view(-1, self.num_generations_grad).mean(dim=1)
        std_grouped_rewards = rewards.view(-1, self.num_generations_grad).std(dim=1)
        mean_grouped_rewards = mean_grouped_rewards.repeat_interleave(self.num_generations_grad, dim=0)
        std_grouped_rewards = std_grouped_rewards.repeat_interleave(self.num_generations_grad, dim=0)
        ...
        prompt_completion_ids = prompt_completion_ids[indices]
        ...
        print(f"Elapsed time to prepare inputs: {elapsed_time:.6f} seconds")
